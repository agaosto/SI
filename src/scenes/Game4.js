...
export default class Game4 extends Phaser.Scene {
    ...
    preload() {
        this.load.image('platform4', 'assets/ground_wood.png')
    ...
    create() {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const platform = this.platforms.create(x, y, 'platform4')
            platform.scale = 0.5
        ...
        this.add.text(240, 0, 'Poziom 4 - długość, metr [m]', {
            fontFamily: 'Bangers',
            fontSize: '24px',
            color: '#000'
        })
        ...
        this.add.text(this.scale.width * 0.5, this.scale.height * 0.25, 'DŁUGOŚĆ,\nMETR', {
            fontFamily: 'Bangers',
            fontSize: '64px',
            color: '#000',
            align: 'center'
    ...
    update() {
        ...
        if(this.itemsCollected > 39) {
            this.cameras.main.fadeOut(1000);
            this.scene.start('game5', {itemsCollected: this.itemsCollected})
        }
    }
    ...
    addItemAbove(sprite) {
        ...
        const item = this.items.get(whereItem, y, 'ruler')
        item.setActive(true)
        item.setVisible(true)
        this.add.existing(item).setScale(0.02)
