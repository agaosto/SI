...
export default class Game3 extends Phaser.Scene {
    ...
    preload() {
        this.load.image('platform3', 'assets/ground_snow.png')
    ...
    create() {
        ...
        this.add.text(240, 0, 'Poziom 3 - czas, sekunda [s]', {
            fontFamily: 'Bangers',
            fontSize: '24px',
            color: '#000'
        })
        ...
        this.add.text(this.scale.width * 0.5, this.scale.height * 0.25, 'CZAS,\nSEKUNDA', {
            fontFamily: 'Bangers',
            fontSize: '64px',
            color: '#000',
            align: 'center'
    ...
    update() {
        ...
        if(this.itemsCollected > 29) {
            this.cameras.main.fadeOut(1000);
            this.scene.start('game4', {itemsCollected: this.itemsCollected})
        }
    }
    ...
    addItemAbove(sprite) {
        ...
        const item = this.items.get(whereItem, y, 'clock')
        item.setActive(true)
        item.setVisible(true)
        this.add.existing(item).setScale(0.1)
