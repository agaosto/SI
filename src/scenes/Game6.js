...
export default class Game6 extends Phaser.Scene {
    ...
    preload() {
        this.load.image('platform6', 'assets/ground_stone.png')
    ...
    create() {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const platform = this.platforms.create(x, y, 'platform6')
            platform.scale = 0.5
        ...
        this.add.text(240, 0, 'Poziom 6 - światłość, kandela [cd]', {
            fontFamily: 'Bangers',
            fontSize: '24px',
            color: '#000'
        })
        ...
        this.add.text(this.scale.width * 0.5, this.scale.height * 0.25, 'ŚWIATŁOŚĆ,\nKANDELA', {
            fontFamily: 'Bangers',
            fontSize: '64px',
            color: '#000',
            align: 'center'
    ...
    update() {
        ...
        if(this.itemsCollected > 59) {
            this.cameras.main.fadeOut(1000);
            this.scene.start('game7', {itemsCollected: this.itemsCollected})
        }
    }
    ...
    addItemAbove(sprite) {
        ...
        const item = this.items.get(whereItem, y, 'light')
        item.setActive(true)
        item.setVisible(true)
        this.add.existing(item).setScale(0.8)
